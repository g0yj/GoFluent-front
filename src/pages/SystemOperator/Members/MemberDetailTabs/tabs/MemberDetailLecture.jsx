 ...
 * 회원상세 > 수강 탭
 */
const MemberDetailLecture = ({ member }) => {
      ...
      <div>
        ...
        <div className="ui-info-table small sp-mt-5">
          <table>
            ...
            <tbody>
              {lectureList?.total !== 0 ? (
                lectureList?.list?.map((lec) => (
                  <tr className={status === 'ATTENDING' ? "bgc-c-orange text-center" : "text-center" } key={`lecture-${lec.id}`}>
                    <td>{lec.id}</td>
                    <td>
                      <Buttons
                    ...
                    <td>
                      <a className="ui-link secondary-high" onClick={() => selectLecture(lec)}>
...
const MemberLecture = ({ member, lecture, goBack }) => {
  ...
  const init = useCallback(async () => {
    const res = await ServiceLecture.getDetailLecture(member.id, lecture.id)
                ...
                <div className="layout-flex gap-20">
                  <div className="gap-s">
                    <input
                      ...
                      value={lectureData?.lessonCount || 0}
                      style={{ width: 70 }}
                    ...
                    return (
                      <div key={i}>
                        <input 
                          ...
                          id={`${option.label}-${i}`} 
                          value={option.value}
                          checked={lectureData?.status === option.value} 
                          name="Clarity" 
                          onChange={({ target: { value } }) =>
                            onChangeLectureData({ status: value })}
                        />
                        <label htmlFor={`${option.label}-${i}`}>{option.label}</label>
                      </div>
                    );
                  })}
                </div>
...
export default MemberDetailLecture;
