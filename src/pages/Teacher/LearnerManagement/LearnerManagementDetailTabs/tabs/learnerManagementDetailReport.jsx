...
const LearnerManagementDetailReport = ({member}) => {
  ...
  }, [member.id, paginationData.page, year, month, date]);
          ...
          <tbody>
            {lessonList?.map((item, index) => (
              <tr className="text-center" key={index}>
                <td>{(total - (paginationData.page -1) * 20) - index}</td>
                <td>
                  {item.date} <br /> {item.startTime} ~ {item.endTime}{" "}
                </td>
                <td>{item.courseName}</td>
                <td>{item.teacherName}</td>
                <td>{item.attendanceStatus}</td>
                <td>
                  <Buttons className="primary xsmall" onClick={() => selectLesson(item)}>
                    {item.report ? "수정" : "등록"}
                  </Buttons>
                </td>
              </tr>
...
export default LearnerManagementDetailReport;
